欢迎来到孤爺仔的Openwrt源码仓库！
=
Welcome to Guyezi's  git source of OpenWrt and packages
=
...
1. **不**要用 **root** 用户进行编译！！！
2. 国内用户编译前最好准备好梯子
3. 默认登陆IP 192.168.10.252, 密码 admin
4. 免责声明：翻阅 [LICENSE](LICENSE) 文件
...
3. 使用 `git clone https://github.com/guyezi/opwrt` 命令下载好源代码，然后 `cd lede` 进入目录
...
本套代码保证肯定可以编译成功。里面包括了 [openwrt](https://github.com/openwrt/openwrt) 官方所有源代码，包括 IPK 的。
...
编译完成后输出路径：```SourcePath```/bin/targets
...
2. Login IP is 192.168.10.252 and login password is "admin".
...
Third, logout of root users. And type this `git clone https://github.com/guyezi/opwrt` in terminal to clone this source.
